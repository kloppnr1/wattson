...
/// <summary>
/// Inbox handler for BRS-003 — Incorrect Supplier Switch.
/// Two scenarios:
/// 1. We're the INITIATOR — we reported the error. DataHub sends RSM-004 (D34 accept / D35 reject).
/// 2. We're the RECIPIENT (previous supplier) — DataHub sends RSM-003 (D07) asking us to resume supply.
///    In WattsOn we auto-accept and recreate the supply.
/// </summary>
internal class Brs003InboxHandler
{
    private readonly ILogger _logger;
    public Brs003InboxHandler(ILogger logger) => _logger = logger;
    ...
    public async Task Handle(WattsOnDbContext db, InboxMessage message, JsonElement payload, CancellationToken ct)
    {
        ...
        {
            case "D34": // Correction accepted — we initiated the reversal
                await HandleCorrectionAccepted(db, payload, gsrnStr, ct);
                break;

            case "D35": // Correction rejected — we initiated the reversal
                await HandleCorrectionRejected(db, payload, gsrnStr, ct);
                break;

            case "D07": // Resume supply request — we're the previous supplier
                await HandleResumeRequest(db, message, payload, gsrnStr, ct);
                break;

            default:
                _logger.LogInformation("BRS-003: Unhandled business reason '{Reason}'", businessReason);
                break;
        }
    }
    ...
    private async Task HandleCorrectionAccepted(WattsOnDbContext db, JsonElement payload, string? gsrnStr, CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning("BRS-003/D34: Missing GSRN");
            return;
        }
        ...
        if (process is null)
        {
            _logger.LogWarning("BRS-003/D34: No active BRS-003 initiator process for GSRN {Gsrn}", gsrnStr);
            return;
        }
        ...
        Brs003Handler.HandleCorrectionAccepted(process, supply, correctionDate);
        _logger.LogInformation("BRS-003: Correction accepted for GSRN {Gsrn} — supply ended", gsrnStr);
    }
    ...
    private async Task HandleCorrectionRejected(WattsOnDbContext db, JsonElement payload, string? gsrnStr, CancellationToken ct)
    {
        if (string.IsNullOrEmpty(gsrnStr))
        {
            _logger.LogWarning("BRS-003/D35: Missing GSRN");
            return;
        }
        ...
        if (process is null)
        {
            _logger.LogWarning("BRS-003/D35: No active BRS-003 initiator process for GSRN {Gsrn}", gsrnStr);
            return;
        }

        var reason = PayloadParser.GetString(payload, "reason") ?? "Afvist af hidtidig leverandør";
        Brs003Handler.HandleCorrectionRejected(process, reason);
        _logger.LogInformation("BRS-003: Correction rejected for GSRN {Gsrn}: {Reason}", gsrnStr, reason);
    }
    ...
    private async Task HandleResumeRequest(WattsOnDbContext db, InboxMessage message, JsonElement payload, string? gsrnStr, CancellationToken ct)
    {
        if (string.IsNullOrEmpty(gsrnStr))
        {
            _logger.LogWarning("BRS-003/D07: Missing GSRN");
            return;
        }
        ...
        if (mp is null)
        {
            _logger.LogWarning("BRS-003/D07: Metering point not found for GSRN {Gsrn}", gsrnStr);
            return;
        }
        ...
        var currentSupplierGlnStr = PayloadParser.GetString(payload, "currentSupplierGln") ?? message.SenderGln;
        var currentSupplierGln = GlnNumber.Create(currentSupplierGlnStr);

        // Find a customer linked to this MP via a previous supply
        ...
        {
            _logger.LogWarning("BRS-003/D07: No customer found for GSRN {Gsrn} — cannot resume supply", gsrnStr);
            return;
        }

        var result = Brs003Handler.HandleResumeRequest(
            gsrn, resumeDate, transactionId, currentSupplierGln, mp, customerEntity);

        db.Processes.Add(result.Process);
        db.Supplies.Add(result.NewSupply);
        _logger.LogInformation("BRS-003: Supply resumed for GSRN {Gsrn} from {Date}", gsrnStr, resumeDate.ToString("yyyy-MM-dd"));
    }
